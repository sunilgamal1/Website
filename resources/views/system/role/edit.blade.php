@if($module['name'] !== 'Dashboard')
    <h4>@if(!$module['hasSubmodules'])
            <input id="{{$module['name']}}" type="checkbox" class="{{str_replace('/','',$module['permissions'][0]['route']['url'])}} module"
                   data-module="{{str_replace('/','',$module['permissions'][0]['route']['url'])}}"
                   @if(in_array(['url' => $module['permissions'][0]['route']['url'], 'method' => 'get'], $item->permissions)) checked @endif>
        @endif
        <label for="{{ $module['name'] }}">{{ translate($module['name']) }}</label>
    </h4>
        ...
        @foreach($module['submodules'] as $submodule)
            <h5><label class="checkbox-inline">
                <input type="checkbox"
                       ...
                       @if(in_array(['url' => $submodule['permissions'][0]['route']['url'], 'method' => 'get'], $item->permissions)) checked @endif>
