                ...
                <div class="content-display clearfix">
                                                    ...
                                                    <form method="post" action="{{url($indexUrl.'/'.$item->id)}}"
                                                        ...
                                                        @elseif($item->isFile($item->type))
                                                            <div style="display:flex;">
                                                                <img src="{{ asset('uploads/config/'.$item->value) }}"
                                                                     class="img-thumbnail mr-2" alt="{{$item->value}}"
                    ...
                    @if(hasPermission($indexUrl, 'post'))
                        <div class="panel panel-default">
                            <div class="panel-heading no-bdr">
                                <form method="post" class="form-inline" action="{{ url($indexUrl) }}"
                                    ...
                                    <div id="dynamic-field-wrapper" class="d-none">
                                        <x-system.form.form-inline-group
                                            :input="['name'=>'value', 'id' => 'text-value', 'label' => 'Value', 'groupId' => 'text-type', 'required' => true, 'default' => old('value'), 'error' => $errors->first('value')]"/>
                                        <x-system.form.form-inline-group
                                            :input="['name'=>'value', 'id' => 'number-value', 'label' => 'Value', 'groupId' => 'number-type', 'type'=>'number', 'required' => true, 'default' => old('value'), 'error' => $errors->first('value')]"/>
                                        <x-system.form.form-inline-group
                                            :input="['name'=>'value', 'label' => 'Select file', 'groupId' => 'file-type']">
                                            <x-slot name="inputs">
                                                <x-system.form.input-file
                                                    :input="['name' => 'value', 'id' => 'file-value', 'label' => 'Select Image', 'accept' => 'image/*', 'required' => true, 'error' => $errors->first('value')]"/>
                                            </x-slot>
                                        </x-system.form.form-inline-group>
                                        <x-system.form.form-inline-group
                                            :input="['name'=>'value', 'label' => 'Value', 'groupId' => 'textarea-type']">
                                            <x-slot name="inputs">
                                                <x-system.form.text-area
                                                    :input="['name' => 'value', 'id' => 'textarea-value', 'label' => 'Value', 'rows' => 2, 'required' => true, 'default' => old('value'), 'error' => $errors->first('value')]"/>
                                            </x-slot>
                                        </x-system.form.form-inline-group>
                                    </div>
